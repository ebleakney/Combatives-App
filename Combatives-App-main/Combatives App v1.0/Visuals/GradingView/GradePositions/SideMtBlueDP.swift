...
struct SideMountBlueDPView: View {
    ...
    @State private var navigateToSideMtGreyView = false // State variable to control navigation to the next view
    ...
    var body: some View {
        VStack {
            ...
            List {
                Section(header: Text("Side Mt (DP)").foregroundColor(.black).padding().background(Color.blue)) {
                    ForEach(dpScoringItems.indices, id: \.self) { index in
                        ScoringRow(item: self.$dpScoringItems[index])
                    }
                }
                Section(header: Text("Side Mt (NDP)").foregroundColor(.black).padding().background(Color.gray)) {
            ...
            HStack {
                ...
                NavigationLink(destination: SideMountGreyDPView(), isActive: $navigateToSideMtGreyView) {
                ...
                Button(action: {
                    ...
                    self.navigateToSideMtGreyView = true
                }) {
...
struct SideMountBlueView_Previews: PreviewProvider {
