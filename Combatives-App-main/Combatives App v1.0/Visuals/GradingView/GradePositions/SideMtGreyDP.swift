...
struct SideMountGreyDPView: View {
    @State private var dpScoringItems: [ScoringItem] = [
        ...
        ScoringItem(description: "Recovered side mount", points: 3, isExtraPoints: false),
        ScoringItem(description: "Maintained", points: 5, isExtraPoints: false),
        ScoringItem(description: "Gained knee mount", points: 7, isExtraPoints: false),
        ScoringItem(description: "Gained ft or rear mount", points: 10, isExtraPoints: false),
    ...
    @State private var navigateToFrontMtBlueView = false // State variable to control navigation to the next view
    ...
    var body: some View {
        VStack {
            ...
            List {
                Section(header: Text("Side Mt (DP)").foregroundColor(.black).padding().background(Color.gray)) {
                    ForEach(dpScoringItems.indices, id: \.self) { index in
                        ScoringRow(item: self.$dpScoringItems[index])
                    }
                }
                Section(header: Text("Side Mt (NDP)").foregroundColor(.black).padding().background(Color.blue)) {
            ...
            HStack {
                ...
                NavigationLink(destination: FrontMountBlueDPView(), isActive: $navigateToFrontMtBlueView) {
                ...
                Button(action: {
                    ...
                    self.navigateToFrontMtBlueView = true
                }) {
...
struct SideMountGreyView_Previews: PreviewProvider {
