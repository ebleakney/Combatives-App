...
struct ClosedGuardGreyDPView: View {
    ...
    @State private var navigateToSideMtBlueView = false // State variable to control navigation to the next view
    ...
    var body: some View {
        VStack {
            ...
            List {
                Section(header: Text("Closed Guard (DP)").foregroundColor(.black).padding().background(Color.gray)) {
                    ForEach(dpScoringItems.indices, id: \.self) { index in
                        ScoringRow(item: self.$dpScoringItems[index])
                    }
                }
                Section(header: Text("Closed Guard (NDP)").foregroundColor(.black).padding().background(Color.blue)) {
            ...
            HStack {
                ...
                NavigationLink(destination: SideMountBlueDPView(), isActive: $navigateToSideMtBlueView) {
                ...
                Button(action: {
                    ...
                    self.navigateToSideMtBlueView = true // Activate navigation to NextView
...
struct ClosedGuardGreyView_Previews: PreviewProvider {
