...
struct FrontMountBlueDPView: View {
    ...
    var body: some View {
        VStack {
            ...
            List {
                Section(header: Text("Front Mt (DP)").foregroundColor(.black).padding().background(Color.blue)) {
                    ForEach(dpScoringItems.indices, id: \.self) { index in
                        ScoringRow(item: self.$dpScoringItems[index])
                    }
                }
                Section(header: Text("Front Mt (NDP)").foregroundColor(.black).padding().background(Color.gray)) {
